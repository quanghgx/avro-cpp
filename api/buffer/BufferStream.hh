...
/* Custom istream and ostream classes for use with buffers*/
namespace avro {
  ...
  /* Custom istream class for reading from an InputBuffer.
     If the buffer contains binary data, then it is recommended to only use the read() and readsome() functions--get() or getline() may be 
     confused if the binary data happens to contain an EOF character. For buffers containing text, the full implementation of istream is safe*/
  ...
  public:
    ...
    /* Constructor, takes an OutputBuffer to read from (by making a shallow copy to an InputBuffer). Writing to the OutputBuffer while an 
       istream is using it may lead to undefined behavior*/
