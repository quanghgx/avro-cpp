...
namespace avro::detail {
  ...
  /* Implementation details for Buffer class
     Internally, BufferImpl keeps two lists of chunks, one list consists entirely of chunks containing data, and one list which contains 
  ...
  class BufferImpl {
    ...
    void postWrite(size_type size) {
      ...
      /* precondition to this function is that the writeChunk_.front()  contains the data that was just written, so make sure writeChunks_ 
         is not empty:*/
      ...
      /* update the states of both readChunks_ and writeChunks_ to indicate that they are holding the new data*/
      readChunks_.back().incrementCursor(size);
      writeChunks_.front().incrementCursor(size);
      size_ += size;
      freeSpace_ -= size;

      /* if there is no more free space in writeChunks_, the next write cannot use  it, so dispose of it now*/
  ...
  public:
    ...
    /* Add enough free chunks to make the reservation size available.  Actual amount may be more (rounded up to next chunk).*/
    ...
    void writeTo(T val, const boost::true_type&) {
      if (freeSpace_ && (sizeof (T) <= writeChunks_.front().freeSize())) {
        // fast path, there's enough room in the writable chunk to just
    ...
    /* Copy data to a different buffer by copying the chunks.  It's  a bit like extract, but without modifying the source buffer.*/
    void copyData(BufferImpl &dest,
      ...
      while (copied < bytes) {
        ...
        /* offset only applies in the first chunk, all subsequent chunks are copied from the start*/
    ...
    /* The number of chunks containing free space (note that an entire chunk  may not be free).  Used for debugging.*/
