...
/* Low level support for encoding avro values. This class has two types of functions.  One type of functions support the writing of leaf 
   ...
   The other type of functions support the writing of maps and arrays. These functions are arrayStart, startItem, and arrayEnd (and similar 
   functions for maps). Some implementations of Encoder handle the buffering required to break large maps and arrays into blocks, which is 
   necessary for applications that want to do streaming.*/
namespace avro {

  /* The abstract base class for all Avro encoders. The implementations differ in the method of encoding (binary versus JSON) or in 
     capabilities such as ability to verify the order of invocation of different functions.*/
  class Encoder {
  public:
    ...
    /* Encodes arbitrary binary data into the current stream as Avro "bytes" data type.
        ...
        @param len Number of bytes at bytes.*/
